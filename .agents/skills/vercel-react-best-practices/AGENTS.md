...
- **Never use fetch and useEffect together.** Use TanStack Query (or similar) for client-side data fetching instead of manual `fetch` inside `useEffect`.
- **Avoid useEffect as much as possible.** Prefer event handlers, deriving state during render, or data-fetching libraries; use `useEffect` only for genuine external synchronization (e.g. DOM, subscriptions).
- **Always use `bun`.** Use the `bun` package manager for installing dependencies and running scripts.

---

## Table of Contents

0. [Project Conventions](#project-conventions)
...
4. [Client-Side Data Fetching](#4-client-side-data-fetching) — **MEDIUM-HIGH**
   ...
   - 4.3 [Use TanStack Query (or SWR) for Client-Side Data Fetching](#43-use-tanstack-query-or-swr-for-client-side-data-fetching)
...
### 4.3 Use TanStack Query (or SWR) for Client-Side Data Fetching

**Impact: MEDIUM-HIGH (automatic deduplication)**

Never combine `fetch` with `useEffect`. Use TanStack Query (or SWR) for client-side data fetching: they provide deduplication, caching, and revalidation without manual effect logic.

**Incorrect: fetch + useEffect (avoid)**
...
**Correct: TanStack Query (preferred)**
...
Reducing unnecessary re-renders minimizes wasted computation and improves UI responsiveness. Prefer event handlers, derived state during render, or data-fetching libraries over `useEffect`; use effects only for genuine external synchronization (DOM, subscriptions).
